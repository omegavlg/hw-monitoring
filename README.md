...
- SLA (Service Level Agreement) в % – можно вычислять на основе успешных HTTP-запросов (2xx).
- Среднее время ответа (HTTP latency) – насколько быстро сервис обрабатывает запросы клиентов.
- Количество успешных и неуспешных вычислений – например, метрика успешных вычислений vs. ошибок.
- Заполненность дискового хранилища – если место заканчивается, платформа может перестать сохранять отчёты.